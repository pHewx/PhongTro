        ...
        {(name === "category" || name === "province") && (
          <div className="p-4 flex flex-col">
            ...
            {content?.map((item) => {
              return (
                <span
                  key={item.code || item.province_id}
                  className="py-2 flex gap-2 items-center border-b border-gray-200"
                >
                  <input
                    type="radio"
                    name={name}
                    id={item.code || item.province_id}
                    value={item.code || item.province_id}
                    checked={
                      item.code || item.province_id === queries[`${name}Code`]
                        ? true
                        : false
                    }
                    onChange={(e) =>
                      handleSubmit(e, {
                        [name]: item.value || item.province_name,
                        [`${name}Code`]: item.code || item.province_id,
                      })
                    }
                  />
                  <label htmlFor={item.code || item.province_id}>
